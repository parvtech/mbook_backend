    ...
    def __call__(self, request):
        ...
        if isinstance(response, Response):
            ...
            if response.status_code in [200, 201]:
                res = BaseResponseSerializer.success_response(response.data, response.status_code).data
            elif response.status_code in [400, 401, 403, 404, 405, 409, 500]:
                msg = {400: constants["BadRequestMessage"],
                ...
                res = BaseResponseSerializer.error_response(response.data, status=response.status_code,
                                                            message=msg[response.status_code]).data
            return HttpResponse(
                json.dumps(res), content_type="application/json", status=response.status_code
            )
        return response
